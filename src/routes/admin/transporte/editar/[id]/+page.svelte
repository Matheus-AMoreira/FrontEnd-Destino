<script lang="ts">
    ...
    let empresa = $state(data.transporte?.empresa || '');
    let meio = $state(data.transporte?.meio || '');
    let preco = $state<number | string>(data.transporte?.preco || '');
    ...
    async function handleSubmit(e: Event) {
        ...
        try {
            await apiFetch(`/admin/transportes/${data.transporte.id}`, {
                method: 'PUT',
                body: JSON.stringify({
                    empresa,
                    meio,
            ...
            if (err.errors) {
                ...
                alert(err.message || 'Erro ao atualizar transporte.');
            }
        } finally {
            processing = false;
        }
...
<AdminLayout title="Editar Transporte">
    <div class="mx-auto max-w-2xl">
        <div class="mb-8 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-2xl font-bold text-gray-900">Editar Transporte</h1>
            </div>
        </div>
