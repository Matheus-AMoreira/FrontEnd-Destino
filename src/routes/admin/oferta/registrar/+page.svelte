<script lang="ts">
    ...
    let disponibilidade = $state<number | string>('');
    let status = $state('DISPONIVEL');
    let pacote_id = $state<number | string>('');
    let hotel_id = $state<number | string>('');
    let transporte_id = $state<number | string>('');
    ...
    async function handleSubmit(e: Event) {
        ...
        try {
            ...
            if (err.errors) {
                ...
                alert(err.message || 'Erro ao salvar oferta.');
            }
        } finally {
            processing = false;
        }
    ...
    <div class="mx-auto max-w-3xl">
        <div class="mb-8 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-2xl font-bold text-gray-900">Nova Oferta</h1>
            </div>
        </div>
