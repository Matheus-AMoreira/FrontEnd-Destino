<script lang="ts">
    ...
    let nome = $state(data.foto?.nome || '');
    let descricao = $state(data.foto?.descricao || '');
    let storage_type = $state(data.foto?.storage_type || 'LOCAL');
    ...
    async function handleSubmit(e: Event) {
        ...
        try {
            await apiFetch(`/admin/pacotedefotos/${data.foto.id}`, {
                method: 'PUT',
                body: JSON.stringify({
                    nome,
                    descricao,
            ...
            if (err.errors) {
                ...
                alert(err.message || 'Erro ao atualizar álbum.');
            }
        } finally {
            processing = false;
        }
    ...
    <div class="mx-auto max-w-2xl">
        <div class="mb-8 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-2xl font-bold text-gray-900">Editar Álbum</h1>
            </div>
        </div>
