<script lang="ts">
    ...
    let nome = $state(data.hotel?.nome || '');
    let diaria = $state<number | string>(data.hotel?.diaria || '');
    let cidade_id = $state<number | string>(data.hotel?.cidade_id || '');
    let endereco = $state(data.hotel?.endereco || '');
    ...
    async function handleSubmit(e: Event) {
        ...
        try {
            await apiFetch(`/admin/hoteis/${data.hotel.id}`, {
                method: 'PUT',
                body: JSON.stringify({
                    nome,
            ...
            if (err.errors) {
                ...
                alert(err.message || 'Erro ao atualizar hotel.');
            }
        } finally {
            processing = false;
        }
...
<AdminLayout title="Editar Hotel">
