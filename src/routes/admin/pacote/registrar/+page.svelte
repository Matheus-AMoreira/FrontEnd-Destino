<script lang="ts">
    ...
    let descricao = $state('');
    let tags = $state('');
    let funcionario_id = $state('');
    let pacote_foto_id = $state('');
    ...
    async function handleSubmit(e: Event) {
        ...
        try {
            await apiFetch('/admin/pacotes', {
                method: 'POST',
                body: JSON.stringify({
                    name: nome,
                    description: descricao,
                    tag_ids: tags,
                    staff_id: funcionario_id,
                    package_photo_id: pacote_foto_id
                })
            });
            goto('/admin/pacote');
        } catch (err: any) {
            if (err.errors) {
                ...
                alert(err.message || 'Erro ao salvar pacote.');
            }
        } finally {
            processing = false;
        }
    ...
    <div class="mx-auto max-w-2xl">
        <div class="mb-8 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-2xl font-bold text-gray-900">Novo Pacote</h1>
            </div>
        </div>
        ...
        <form onsubmit={handleSubmit} class="space-y-6 rounded-2xl bg-white p-8 shadow-sm border border-gray-100">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    ...
                    {#if errors.funcionario_id}<p class="mt-1 text-xs text-red-500">{errors.funcionario_id}</p>{/if}
                </div>
                ...
                <div>
                    ...
                    {#if errors.pacote_foto_id}<p class="mt-1 text-xs text-red-500">{errors.pacote_foto_id}</p>{/if}
                </div>
            </div>
            ...
            <div class="pt-6 border-t border-gray-100">
                <div>
                    ...
                    />
                    {#if errors.tags}<p class="mt-1 text-xs text-red-500">{errors.tags}</p>{/if}
