...
export default function MinhasViagens() {
  ...
  const activeView = searchParams?.view || "andamento";
  const listaAtual =
    ...
    const fetchTodasViagens = async () => {
      ...
      try {
        const [resAndamento, resConcluidas] = await Promise.all([
          api.get("/compra/andamento"),
          api.get("/compra/concluidas"),
        ]);

        setViagensAndamento(resAndamento.data);
        setViagensConcluidas(resConcluidas.data);
      } catch (error) {
  ...
  const handleMudarVisualizacao = (view: string) => {
    navigate({ search: { view }, replace: true } as any);
  };
                        ...
                        <button
                          onClick={() =>
                            navigate({
                              ...
                              params: {
                                usuario: usuario?.nomeCompleto || "usuario",
                                id: String(viagem.id),
                              },
                            })
                          }
