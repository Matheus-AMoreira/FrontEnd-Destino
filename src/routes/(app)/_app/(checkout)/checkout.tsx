...
export const Route = createFileRoute("/(app)/_app/(checkout)/checkout")({
...
export default function CheckoutPage() {
  ...
  const pacoteState =
    (location.state as any)?.pacote || (window.history.state as any)?.pacote;
  const ofertaState =
    (location.state as any)?.oferta || (window.history.state as any)?.oferta;
  ...
  useEffect(() => {
    if (!pacoteState || !ofertaState) {
      setModal({
        show: true,
        mensagem: "Nenhum pacote ou data selecionados.",
        url: "/buscar",
      });
    }
  }, []);
  ...
  // Cálculos
  const valorTotal = ofertaState.preco || 0;
  ...
  const handleFinalizarCompra = async () => {
    ...
    try {
      ...
      const result = response.data;
        ...
        navigate({
        to: "/checkout/confirmacao/$compraId",
        params: { compraId: String(result.compra.id) },
      });
    } catch (error: any) {
      console.error(error);
      const msg =
        error.response?.data?.mensagem ||
        "Erro de conexão ao processar compra.";
      setModal({
        show: true,
        mensagem: `Erro na compra: ${msg}`,
        url: null,
      });
    } finally {
      setLoading(false);
            ...
            <div className="bg-white rounded-lg shadow-md border border-gray-200 p-6 sticky top-4">
              ...
              <div className="space-y-3 mb-4">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span>
                    {ofertaState.hotel?.cidade?.nome} -{" "}
                    {ofertaState.hotel?.cidade?.estado?.sigla}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
